...
**Мастерчейн** – единственная в своём роде цепочка блоков, содержащая общую информацию о протоколе и текущих значениях его параметров, набор валидаторов и их долей, набор активных в данный момент воркчейнов и их «шардов», а также набор хэшей последних блоков мастерчейнов и шардчейнов.  
  
**Воркчейн** \(Working blockchains\) — множество блокчейнов, которые являются «рабочими лошадками», содержащими транзакции по перемещению активов и смарт-контракты. При этом отдельные воркчейны могут иметь свои собственные «правила», форматы адресов аккаунтов, форматы транзакций, различные виртуальные машины для смарт-контрактов, разные базовые токены или криптовалюты. Тем не менее все они должны удовлетворять основным критериям функциональной совместимости.

### Что такое шардинг?

Воркчейны в свою очередь разделяются на **шарды** или **шардчейны** \(shardchain\). 

Они представляют из себя подмножество блокчейнов внутри множества воркчейнов, обеспечивающее работу системы шардинга и имеющее те же правила и формат блоков, что и у воркчейнов.
